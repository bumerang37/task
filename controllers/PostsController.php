...
class PostsController extends Controller
{
    ...
    {
        //TODO: Отрефакторить метод, разбить на составные части
        $titles = [
            'Русский' => [
        ...
        $lang = Languages::find()->select(['id', 'title'])->orderBy(['RAND()' => SORT_ASC])->one();
        $obj['lang'] = $lang->id;
        ...
        $sentence = 0; //Количество предложений
        $sentence = rand(3, 4);

        if ($lang->title == 'Русский') {
            $text = $texts['Русский']; //Текст с которым будем работать
            $title = $titles['Русский'];
            $str = '';

            $titleRand = array_rand($title, $titleWords);
            ...
            for ($j = 0; $j < $sentence; $j++) {
                ...
                $text_rand = array_rand($text, $textWords);
